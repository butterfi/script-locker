...
DBNAMES="drupal"
MAXATTSIZE="4000"
MAILADDR="dan.shumaker@edutopia.org"
TBEXCLUDE="drupal.sessions drupal.accesslog drupal.cache drupal.cache_form drupal.cache_apachesolr drupal.cache_block drupal.cache_content drupal.cache_filter drupal.cache_menu drupal.cache_page drupal.cache_update drupal.cache_views drupal.cache_views_data drupal.notifications drupal.notifications_event drupal.notifications_fields drupal.notifications_queue drupal.notifications_sent"
EXCLUDE_DUMP=1
MDBNAMES="$DBNAMES"
DBEXCLUDE=""
CREATE_DATABASE=yes
...
OOPT=$OPT			# This string is for the schema dump
EXD=" "
for table in $TBEXCLUDE
do
    OPT="$OPT --ignore-table=$table"
    if [ $EXCLUDE_DUMP ]; then
        EXD="$EXD ${table##*.}"
    fi 
done
...
dbdump () {
    if [ $addschema ]; then
        if [ $EXCLUDE_DUMP ]; then
            mysqldump --user=$USERNAME --password=$PASSWORD --host=$DBHOST $OOPT $1 $EXD > ${2}.exclude_dump.sql
        fi
